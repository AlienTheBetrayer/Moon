...
bool Moon::Misc::IsWithinRect(const Vector2I& pos, const RectI& rect) noexcept
{
	return pos.x >= rect.left && pos.x <= rect.right && pos.y >= rect.top && pos.y <= rect.bottom;
}

Vector2I Moon::Misc::GetCursorPosition(void) noexcept
{
	POINT point;
	GetCursorPos(&point);
...
Vector2I Moon::Misc::SnapToConsole(const Vector2I& position) noexcept
{
	POINT point;
	point.x = position.x;
	point.y = position.y;
